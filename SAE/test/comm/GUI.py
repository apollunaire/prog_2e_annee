...
import csv
import os
from client_asyn import Client
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.__bConnect.clicked.connect(self.__actionConnect)
    ...
    def __actionConnect(self):
        ...
        print(m)
        self.client = Client(host=f"{m}", port=15001)
        self.client.connect()
        print("connexion OK")
    ...
    def __actionCPU(self):
        m = self.__choixM.currentText()
        self.__sortieB.setText(f"{self.client.get_CPU()}")
    def __actionRAM(self):
        m = self.__choixM.currentText()
        self.__sortieB.setText(f"{self.client.get_RAM()}")
    def __actionIP(self):
        m = self.__choixM.currentText()
        self.__sortieB.setText(f"{self.client.get_IP()}")
    def __actionOS(self):
        m = self.__choixM.currentText()
        self.__sortieB.setText(f"{self.client.get_OS()}")
    def __actionName(self):
        m = self.__choixM.currentText()
        self.__sortieB.setText(f"{self.client.get_hostname()}")
    ...
    def __actionDis(self):
        m = self.__choixM.currentText()
        disconnect(m)
        self.client.dialogue("disconnect")


    def __actionKill(self):
        m = self.__choixM.currentText()
        kill(m)
        self.client.dialogue("kill")


    def __actionReset(self):
        m = self.__choixM.currentText()
        reset(m)
        self.client.dialogue("reset")
